...
//读取用户购物记录文件
void Shopping::read_shopping_history(user* this_user)
{
	this_user->history_num  = 0;//先将购买记录数量置为0
	string shopping_history_address = this_user->name + "_history.txt";//设置购买记录文件地址

	ifstream in_file;
	in_file.open(shopping_history_address, ios::in);
	...
	while (in_file >> this_user->history[this_user->history_num].id &&
		in_file >> this_user->history[this_user->history_num].name &&
		in_file >> this_user->history[this_user->history_num].brand &&
		in_file >> this_user->history[this_user->history_num].price &&
		in_file >> this_user->history[this_user->history_num].amount &&
		in_file >> this_user->history[this_user->history_num].time) //读入库存数据
	{
		this_user->history_num++;
	}
	in_file.close();
}

//写入用户购物记录文件
void Shopping::write_shopping_history(user* this_user)
{
	//更新的用户购物记录写入用户购物记录文件
	string shopping_history_address = this_user->name + "_history.txt"; //设置购物车文件地址

	ofstream out_file;
	out_file.open(shopping_history_address, ios::out);	//以写方式创建并打开
	out_file << "id\t" << "名称\t" << "品牌\t" << "价格\t" << "购买数量\t" << "购买时间" << endl;

	if (this_user->history_num != 0)
	{
		for (int i = 0; i < this_user->history_num; i++)
		{
			out_file << this_user->history[i].id << "\t"
				<< this_user->history[i].name << "\t"
				<< this_user->history[i].brand << "\t"
				<< this_user->history[i].price << "\t"
				<< this_user->history[i].amount << "\t"
				<< this_user->history[i].time << endl;
		}
	}	
	out_file.close();
}
...
void Shopping::user_operate(int index, user* this_user)
{
	...
	cout << "6.查看购物车     7.结账     8.修改密码	9.修改购物车商品数量	10.查看购物历史记录" << endl;
	cout << "==============================================================================" << endl;
	...
	while (n != 1)
	{
		...
		case 9:
			...
			break;
		case 10:
			Shopping::visit_shopping_history(index, this_user);
			break;
		default:
			cout << "错误的操作指令！" << endl;
			break;
...
//查看购物历史记录
void  Shopping::visit_shopping_history(int index, user* this_user)
{
	read_shopping_history(this_user); //读入购物记录文件

	cout << "请输入要查找记录的时间：" << endl;
	cout << "年：" ;
	int year;
	cin >> year;
	cout << "月：";
	...
	{
		cout << "--------------------------------------------------------" << endl;
		cout << "id\t" << "名称\t" << "品牌\t" << "价格\t" << "购买数量\t" << "购买时间" << endl;
		int MAX_num = 0;
		int flag = 0;
		int money = 0;
		for (int i = 0; i < this_user->history_num; i++)
		{
			if (this_user->history[i].time == time)
			{
				if (this_user->history[i].amount > MAX_num)
				{
					MAX_num = this_user->history[i].amount;
					flag = i;
				}				
				cout << this_user->history[i].id << "\t"
					<< this_user->history[i].name << "\t"
					<< this_user->history[i].brand << "\t"
					<< this_user->history[i].price << "\t"
					<< this_user->history[i].amount << "\t"
					<< this_user->history[i].time << "\t" << endl;
				money += this_user->history[i].price * this_user->history[i].amount;
			}
		}
		cout << "--------------------------------------------------------" << endl;
		cout << "该时间内用户购买商品的总金额为 " << (money*0.9) << endl;
		cout << "经分析，该时间内用户 " << this_user->name << " 最常购买的物品为 " << this_user->history[flag].brand << " 牌子的 " << this_user->history[flag].name << endl;
	}
...
void Shopping::check_out(int index, user* this_user)
{
	...
	read_shopping_history(this_user);
	...
	if (this_user->shopping_car_num > 0)
	{
		...
		else if (choice == 1)
		{
			...
			{				
				...
				//将购物车数组中的数据全部复制到购物历史数组中
				this_user->history[this_user->history_num].id = this_user->shopping_car[i].id;
				this_user->history[this_user->history_num].name = this_user->shopping_car[i].name;
				this_user->history[this_user->history_num].brand = this_user->shopping_car[i].brand;
				this_user->history[this_user->history_num].price = this_user->shopping_car[i].price;
				this_user->history[this_user->history_num].amount = this_user->shopping_car[i].amount;
				this_user->history[this_user->history_num].time = time;

				this_user->history_num++;
				cout << this_user->history_num << endl;
			}
			//清空用户购物车
			this_user->shopping_car_num = 0;

			write_shopping_history(this_user);	//写入购物记录文件
			...
			while (1)
			{
				...
				for (i = 0; i < this_user->shopping_car_num; i++)
				{
					if (this_user->shopping_car[i].id == id)
					{
						...
						if (this_user->shopping_car[i].amount > num)
						{
							//将购物车数组中的这个物品的数据复制到购物历史数组中
							this_user->history[this_user->history_num].id = this_user->shopping_car[i].id;
							this_user->history[this_user->history_num].name = this_user->shopping_car[i].name;
							this_user->history[this_user->history_num].brand = this_user->shopping_car[i].brand;
							this_user->history[this_user->history_num].price = this_user->shopping_car[i].price;
							this_user->history[this_user->history_num].amount = num;
							this_user->history[this_user->history_num].time = time;
							this_user->history_num++;
